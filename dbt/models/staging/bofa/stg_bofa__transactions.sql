...
with
    transactions as (
        ...
        select
            ...
            substr(
                account_name,
                instr(account_name, '-') + 2,
                instr(substr(account_name, instr(account_name, '-') + 1), '-') - 3
            ) as account_type,
            substr(
                account_name,
                instr(account_name, '-')
                + instr(substr(account_name, instr(account_name, '-') + 1), '-')
                + 2
            ) as account_name,
            ...
            {{ standardise_ascii('trim(original_description)') }}
    ...
    numbered as (
        ...
        select
            *,
            row_number() over (
                partition by
                    account_name, transaction_date, original_description, amount
    ...
    flagged as (
        ...
        from keyed as k
        left join transfers as t on k.transaction_key = t.transaction_key
