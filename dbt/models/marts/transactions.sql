...
with
    revolut as (
        ...
        select
            ...
            is_transfer
        from {{ ref('stg_revolut__transactions') }}
    ...
    bofa as (
        ...
        select
            ...
            is_transfer
        from {{ ref('stg_bofa__transactions') }}
    ...
    translated as (
        ...
        select
            ...
            case when c.currency = 'EUR' then gf.rate else 1 end as fx_rate,
            round(
                case when c.currency = 'EUR' then c.amount * gf.rate else c.amount end,
                2
            ) as amount_usd
        from combined as c
        left join
        ...
        left join
            {{ make_seed('category_mapping') }} as cm
            on c.category = case
